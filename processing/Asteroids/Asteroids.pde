...
void oscEvent(OscMessage m) {
  ...
  float acceleration = m.get(1).floatValue();
...
void draw(){
 ...
 if( lives >= 0 && asteroids.size()>0){
   ...
   if(upPressed){
     ship.acceleration = new PVector(0,shipSpeed * shipAcceleration); 
   ...
   textFont(font, 22);
   text("High Score " + highScore, width * .4, 50);
   fill(255);
   ...
   if(darkCounter < darkCounterLimit){
    ...
    rect(0,0,width,height);
     ...
     String score = "" + highScore;
      ...
      list[0] = score;
      saveStrings(highScoreFileName, list);
